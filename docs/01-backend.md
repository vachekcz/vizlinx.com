...
- **Identita a rozpočet webu:** `crawl_frontier.origin` zůstává logickým vlastníkem i pro URL na potvrzeném aliasu; skutečný síťový origin je v `url`. Alias tak sdílí limit 1 000 stránek, interval a pauzu a v mapě patří stejné kouli. Dva originy výslovně zadané uživatelem zůstávají samostatné. Pokud redirect dodatečně připojí existující preview, jeho rezervované sloty i původní výsledky zůstanou zachovány; již uložené odkazy způsobilých běžných a preview výsledků se rozvinou bez opakovaného stažení stránky. Ruční výsledky si zachovají rozsah jedné URL a své odkazy ani přesměrování automaticky nerozvíjejí. Případné předchozí HTTP 429 přenese pauzu také na nového vlastníka. Změna vlastníka je atomická s uloženým důkazem přesměrování a chráněná generací/lease.
...
Obnovení přes `/start` zachová frontier i příznak omezení HTTP 429. Z uložených výsledků znovu doplní jen povolenou práci: HTML odkazy z dosud externích preview výsledků další sken nerozvíjejí. **Povýšení webu** používá existující `POST /scans/:id/sites` a následný `/start`; dosavadní výsledky a sloty zůstávají, uložené odkazy jeho preview výsledků už mohou pokračovat běžným skenem. Ruční výsledky si i po povýšení zachovají rozsah jedné URL. Dřívější kontroly se započítají do jeho limitu 1 000 stránek a URL se znovu nestahují. `/rescan` založí nový průchod s prázdným frontierem a robots, tedy i novými kvótami kontrol; předchozí výsledky a log zůstanou v archivu.
...
Akce rezervuje `crawl_frontier.is_manual = 1`, zajistí robots checkpoint a spustí frontu ve stejné mapě. Nemění `sites_json`, proto ji neomezuje počet tří webů ani kvóty automatických kontrol 10/100. URL na serverem potvrzeném aliasu patří stejnému logickému vlastníkovi jako ostatní stránky tohoto webu. Celkový limit **1 000 stránek na web včetně potvrzených aliasů** zahrnuje všechny výsledky, neúspěšné pokusy i rezervované ruční stránky; opakované kliknutí slot nepřidává a hotový výsledek znovu nestahuje. Dříve rezervovaný preview slot zůstává obsazený i po ručním výběru.
...
| Stránky na logického vlastníka webu (včetně aliasů, neúspěšných pokusů, dřívějších kontrol a ručních skenů) | 1 000 | `SCAN_LIMITS.pagesPerSite` |
| Automaticky kontrolované externí URL na web včetně aliasů / průchod mapou | 10 / 100 | `SCAN_LIMITS.previewPagesPerSite`, `.previewPagesPerScan`; trvalé sloty ve frontieru |
