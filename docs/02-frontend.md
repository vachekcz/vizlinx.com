...
- **Porovnání hover efektů:** `/ux/2?hover=pulse`, `?hover=quiet` a `?hover=focus` nabízejí tři interaktivní varianty (jednorázový puls a průběh po spojnici, statická záře, zvýraznění s utlumením okolí). Přepínač mění query bez resetu mapy. Bez parametru je porovnání vypnuté; otevře ho „Varianty hoveru“ v liště studie. Hover/fokus řádku ukazuje konkrétní web nebo směrové propojení bez změny výběru, historie detailu, rozložení a přiblížení; odkaz ven a akce skenu jsou samostatné. Při zvýraznění propojení mají obrysy, záře a puls obou koncových webů poloviční opacity oproti zvýraznění samotného webu, aby vynikla spojnice. Platí pro hover, přehrání i otevřený detail; intenzita spojnice a samostatného webu zůstává stejná. Rozbalené weby zvýrazňují odpovídající jednotlivé spojnice. Tlačítka ukázky přehrají efekt na 3 sekundy; na mobilu dočasně odkryjí mapu a pak obnoví panel. Omezený pohyb vypne puls/průběh a ponechá statické zvýraznění. `Graph` přijímá volitelný `highlight` oddělený od `selection`; používají ho pouze tyto mockupy, vzhled efektů je v `src/ux/hover-studies.css`.
