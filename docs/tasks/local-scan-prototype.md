...
Webové mutace vyžadují stejný Origin a JSON. Runner používá Bearer token; API nemá veřejné CORS pro cizí webové stránky. Autorizace runneru se ověřuje také atomicky při zápisu výsledků, stavu a časů; rozpracovaný požadavek po rotaci či expiraci tokenu nemůže zapisovat. Rozšíření se páruje pouze z vizlinx.com (localhost jen v dev sestavení). Bridge `{type:'vizlinx:ping'}` vrací `{ok:true,protocolVersion:2}`, `{type:'vizlinx:pair',ticket:string}` vrací `{ok:true}`. Starší rozšíření web vyzve k aktualizaci. Server origin je odvozen z ověřeného sender.origin, nikdy z libovolného payloadu. Service worker rozšíření otevře runner.html; runtime permissions si uživatel potvrzuje kliknutím v této kartě.

Upload při dosažení kvóty vrací 429 s `code: 'page_limit'` a aktuálním `maxPages`, nebo `code: 'scan_storage_limit'`. Stav mapy přejde atomicky na limited (existující webová pauza má přednost). Rozšíření uchová odmítnutý výsledek a důvod limitu i přes reload a nové párování. Opakované odeslání blokuje do ověřeného zvýšení limitu stránek; při zaplněné mapě vyžaduje novou mapu. Limit stránek lze změnit v detailu webu až do 50. Překrývající se párování rozšíření odmítne před výměnou tokenu, takže si souběžná spojení nepřepisují relaci.
