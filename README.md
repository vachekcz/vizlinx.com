...
Limit běžného skenu je pevně 100 stránek na origin včetně neúspěšných pokusů. Při dosažení se zobrazí výrazná hláška a výzva kontaktovat správce kvůli vyššímu limitu. Uživatelské API limit nezvýší. `ADMIN_EMAIL` ve Wrangler konfiguraci zapíná kontaktní tlačítko; prázdná hodnota ponechá textovou výzvu bez vymyšlené adresy. Denní nebo časový limit a zaplnění mapy mají vlastní důvod zastavení.

Skener respektuje robots.txt pro `VizlinxBot`, čte HTML do 2 MiB a neposílá cookies. Přesměrování stránky na jinou cestu stejného originu (stejný protokol, hostitel a port) automaticky zařadí do fronty. Každý krok podléhá rychlosti, robots a příslušnému limitu URL; již známá URL se znovu nestahuje. Při automatické kontrole externích cílů přesměrování spotřebovává stejný rozpočet 10/100 URL. Změnu domény, subdomény, protokolu nebo portu nenásleduje a cílovou adresu ukáže v logu a podrobnostech výsledků. To platí i pro přesměrování na další již přidaný web. Automatické kontroly sdílejí časový, denní i úložný rozpočet s běžným skenem. Mapa uchovává výsledky 30 dní od založení a patří anonymní relaci stejného prohlížeče. Samotné předání URL nezpřístupní mapu jiným lidem.
